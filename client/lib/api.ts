...
export const authApi = {
  ...
  ): Promise<{ success: boolean; tenantId: string; tenantName: string }> => {
    const response = await api.post<{ success: boolean; tenantId: string; tenantName: string }>(
...
export const platformApi = {
  ...
  createTenant: async (data: CreateTenantRequest): Promise<CreateTenantResponse> => {
    const response = await api.post<CreateTenantResponse>('/platform/tenants', data);
    return response.data;
  },
  ...
  updateTenantStatus: async (
    ...
    const response = await api.patch<PlatformTenant>(`/platform/tenants/${tenantId}/status`, {
      status,
    });
    return response.data;
  },
...
export const branchesApi = {
  ...
  update: async (branchId: string, data: UpdateBranchRequest): Promise<Branch> => {
    const response = await api.patch<Branch>(`/branches/${branchId}`, data);
    return response.data;
  },

  remove: async (branchId: string): Promise<{ success: boolean }> => {
    const response = await api.delete<{ success: boolean }>(`/branches/${branchId}`);
    return response.data;
  },
};
